...
export default function LegacyStrategyLabPage({ onBackHome }: Props) {
  ...
  const fieldCls =
    'mt-1.5 w-full rounded-xl border border-line bg-[#070d18]/70 px-3.5 py-2.5 text-sm text-ink outline-none transition focus:border-blue/70 focus:ring-2 focus:ring-blue/20'

  return (
    <div className="mx-auto min-h-screen max-w-6xl px-5 pb-20 pt-8 sm:px-8">
      <header className="mb-8 rounded-2xl border border-line bg-panel/90 px-6 py-7 backdrop-blur-xl sm:px-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="text-[1.65rem] font-semibold tracking-tight text-ink sm:text-3xl">
              旧版策略实验室
            </p>
            <h1 className="mt-1.5 text-[0.95rem] text-muted sm:text-base">
              IgniteQuant · 回测控制台（兼容）
            </h1>
            <p className="mt-2 max-w-xl text-sm leading-relaxed text-faint">
              选择策略与标的，提交回测。结果保存在本地，可对照评分与指标。
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3 text-xs text-muted">
            <span
              className={cn(
                ...
                apiOk ? 'border-blue/35 bg-blue-soft text-blue' : 'border-line bg-surface text-muted',
              )}
            >
              <span
                className={cn(
                  'size-1.5 rounded-full',
                  apiOk ? 'bg-blue' : apiOk === false ? 'bg-bad' : 'bg-faint',
                )}
              />
              API {apiOk ? '已连接' : apiOk === false ? '未连接' : '检测中'}
            </span>
            <span className="text-faint">:5173 · :8787</span>
          </div>
        </div>
      </header>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        {[
          ...
          >
            <p className="text-xs tracking-wide text-faint">{stat.label}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
        <aside className="rounded-2xl border border-line bg-panel/90 backdrop-blur-xl">
