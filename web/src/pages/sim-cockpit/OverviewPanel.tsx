...
export function OverviewPanel() {
  ...
  const pipeline = latest
    ? [
        {
          step: '因子',
          detail: `${regimeLabel(latest.regime)} · ${qualityLabel(latest.factor_quality)}`,
        },
        {
          step: '信号',
    ...
    <div className="flex flex-col gap-3">
      ...
      <section className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-line bg-line/40 sm:grid-cols-4 lg:grid-cols-8">
        {[
          ...
          />,
          <Metric key="net" label="净仓" value={net} />,
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        <Section
          ...
          extra={
            latest?.created_at ? (
              <span className="tabular-nums">{latest.created_at}</span>
            ) : null
          }
        >
          {pipeline.length ? (
            ...
            columns={[
              {
                ...
                width: 148,
                render: (v: string) => (
                  <span className="text-[11px] tabular-nums">{v?.slice(5, 19) || '—'}</span>
                ),
              },
              {
                title: '状态',
