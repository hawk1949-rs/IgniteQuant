...
> **实现阶段说明（2026-07）**：当前代码处于「本地作坊 + 云骨架」阶段。座舱与策略实验室仍主要读本地 API/SQLite；Supabase 用于归档、研究表与 outbox 投影。公开 `/research` 路由尚未实现。
...
| `#/sim` 座舱 | 云投影 + 可选本地 API | **本地 SQLite API**（`dashboard/sim_api.py`） |
...
1. `.env`（`TQ_*`、`DATABASE_URL`、`SUPABASE_OWNER_ID`）
2. `data/runtime/*.sqlite` + `*.klines.json`（模拟会话连续性）
